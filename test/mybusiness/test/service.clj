...
(deftest service-test
  ...
  (testing "use boolean well"
    ...
    (deleteTodo (get (first (json/read-str (findTodos (System/getenv "MONGOHQ_URL")) :key-fn #(clojure.string/replace % #"\$\$hashKey" "hashKey")))"_id" ) (System/getenv "MONGOHQ_URL"))
    (is (= 1 (count (json/read-str (findTodos (System/getenv "MONGOHQ_URL")) :key-fn #(clojure.string/replace % #"\$\$hashKey" "hashKey")))))
    )
  )
